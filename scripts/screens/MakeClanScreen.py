...
class MakeClanScreen(Screens):
    ...
    classic_mode_text = (
        "This mode is Flock Generator at it's most basic. "
        "The player will not be expected to manage the minutia of Flock life. <br><br>"
        "Perfect for a relaxing game session or for focusing on storytelling. <br><br>"
        "With this mode you are the eye in the sky, watching the Flock as their story unfolds. "
    )
    ...
    expanded_mode_text = (
        ...
        "With this mode you'll be making the important Flock-life decisions."
    )
    ...
    def screen_switches(self):
        ...
        self.menu_warning = pygame_gui.elements.UITextBox(
            "Note: going back to main menu resets the generated dragonets.",
    ...
    def handle_name_clan_event(self, event):
        if event.ui_element == self.elements["random"]:
            ...
            if not new_name:
                self.elements["error"].set_text("Your Flock's name cannot be empty")
                self.elements["error"].show()
                return
            if new_name.casefold() in [
                clan.casefold() for clan in game.switches["clan_list"]
            ]:
                self.elements["error"].set_text("A Flock with that name already exists.")
                self.elements["error"].show()
                return
            ...
            if not self.elements["name_entry"].is_focused:
                ...
                if new_name.casefold() in [
                    ...
                    self.elements["error"].set_text(
                        "A Flock with that name already exists."
                    )
                    self.elements["error"].show()
                    return
                self.clan_name = new_name
    ...
    def on_use(self):
        ...
        if self.sub_screen == "name clan":
            if self.elements["name_entry"].get_text() == "":
                ...
                self.elements["error"].set_text("Flock names cannot start with a space.")
                self.elements["error"].show()
                self.elements["next_step"].disable()
    ...
    def random_clan_name(self):
        ...
        while True:
            ...
            if chosen_name.casefold() not in [clan.casefold() for clan in game.switches['clan_list']]:
                return chosen_name
            print("Generated flock name was already in use! Rerolling...")
    ...
    def open_game_mode(self):
        ...
        self.elements["permi_warning"] = pygame_gui.elements.UITextBox(
            "Your Flock's game mode is permanent and cannot be changed after Flock creation.",
        ...
        self.elements['random_clan_checkbox'] = UIImageButton(
          ...
          manager=MANAGER,
          tool_tip_text= "When checked, a completely random Flock starting in Newleaf will be generated."
        )

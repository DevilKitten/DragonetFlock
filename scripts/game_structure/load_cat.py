...
def json_load():
    ...
    for cat in all_cats:
        ...
        # Handling wingless dragons from old saves! Because why not
        if cat.pelt.wingless and "lost their wings" not in cat.permanent_condition:
            cat.get_permanent_condition("lost their wings")
        elif "lost their wings" in cat.permanent_condition and not cat.pelt.wingless:
            cat.pelt.wingless = True
    ...
    if len(cat_data) > 0:
        ...
        for i in cat_data.split("\n"):
            ...
            if i.strip() != "":
                ...
                if len(attr) > 37:
                    the_cat.pelt.wingless = bool(attr[37])
                if len(attr) > 39:
